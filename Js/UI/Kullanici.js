...
function getApiBaseUrl() {
    ...
    async function fetchUsers() {
        try {
            const response = await KullaniciEndpoint.request(`${API_BASE_URL}/Users`);
            if (response.ok) {
        ...
        users.forEach(user => {
            ...
            deleteBtn.addEventListener('click', async () => {
                if (confirm(`"${user.username}" kullanıcısını silmek istediğinize emin misiniz?`)) {
                    try {
                        const res = await KullaniciEndpoint.request(`${API_BASE_URL}/Users/${user.id}`, { method: 'DELETE' });
                        if (res.ok) {
    ...
    if (saveRoleBtn) {
        saveRoleBtn.addEventListener('click', async () => {
            ...
            try {
                const response = await KullaniciEndpoint.request(`${API_BASE_URL}/Users/${userId}/role`, {
                    method: 'PUT',
                    headers: {
    ...
    async function loadRecentActivity() {
        ...
        try {
            const res = await KullaniciEndpoint.request(`${API_BASE_URL}/systemlogs?eventType=Authentication&pageSize=10`);
