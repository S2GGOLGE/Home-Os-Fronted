...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function fetchAccountSettings() {
        try {
            const response = await fetch(`${API_BASE}/settings/account`, { headers: getAuthHeaders() });
            if (response.ok) {
                const result = await response.json();
                const data = result.data || result;
                if (data) {
    ...
    if (settingsForm) {
        settingsForm.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                const generalRes = await fetch(`${API_BASE}/Settings`, {
                    method: 'POST',
                ...
                if (newPass && newPass.trim() !== "") {
                    ...
                    if (currentPass) {
                        const passRes = await fetch(`${API_BASE}/settings/password`, {
                            method: 'PUT',
                            headers: getAuthHeaders(),
                            body: JSON.stringify({
                ...
                await fetch(`${API_BASE}/settings/security`, {
                    method: 'PUT',
                    headers: getAuthHeaders(),
                    body: JSON.stringify({
    ...
    if (btnBackup) {
        btnBackup.addEventListener("click", async () => {
            try {
                ...
                const res = await fetch(`${API_BASE}/Settings/backup`, { method: 'POST', headers: getAuthHeaders() });
        ...
        btnRestore.addEventListener("click", async () => {
            if (confirm("Sistemi son alınan veritabanı yedeğine geri yüklemek istediğinize emin misiniz?")) {
                try {
                    ...
                    const res = await fetch(`${API_BASE}/Settings/restore`, { method: 'POST', headers: getAuthHeaders() });
                    const data = await res.json();
    ...
    if (btnPing) {
        btnPing.addEventListener("click", async () => {
            try {
                ...
                const res = await fetch(`${API_BASE}/Settings/ping`, { headers: getAuthHeaders() });
                const data = await res.json();
    ...
    async function fetchMonitoringData() {
        try {
            const res = await fetch(`${API_BASE}/Settings/monitoring`, { headers: getAuthHeaders() });
            if (res.ok) {
