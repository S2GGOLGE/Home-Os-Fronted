...
async function fetchStats() {
    try {
        const res = await BildirimlerEndpoint.request(`${API_BASE}/Notifications/stats`);
        if (!res.ok) return;
...
async function fetchNotifications() {
    try {
        const res = await BildirimlerEndpoint.request(`${API_BASE}/Notifications`);
...
async function markRead(id) {
    try {
        await BildirimlerEndpoint.request(`${API_BASE}/Notifications/${id}/read`, { method: 'PUT' });
...
async function deleteNotif(id) {
    try {
        await BildirimlerEndpoint.request(`${API_BASE}/Notifications/${id}`, { method: 'DELETE' });
...
async function markAllRead() {
    try {
        await BildirimlerEndpoint.request(`${API_BASE}/Notifications/readall`, { method: 'PUT' });
...
async function clearAll() {
    try {
        await BildirimlerEndpoint.request(`${API_BASE}/Notifications/clearall`, { method: 'DELETE' });
        allNotifs = [];
        applyFiltersAndRender();
        fetchStats();
    } catch (e) { console.error(e); }
